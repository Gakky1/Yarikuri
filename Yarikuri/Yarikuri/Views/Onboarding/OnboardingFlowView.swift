...
struct OnboardingFlowView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                // ナビゲーションボタン（concern・customizeは各ステップ内に独自ボタンあり）
                if vm.currentStep != .welcome && vm.currentStep != .concern && vm.currentStep != .customize {
                    navigationButtons
                        .padding(.horizontal, 24)
                        .padding(.bottom, 40)
                }
    ...
    private var stepContent: some View {
        ...
        case .concern:
            ConcernStepView(vm: vm, onComplete: { vm.nextStep() })
        case .customize:
            CustomizeStepView(vm: vm, onComplete: completeOnboarding)
        }
    }
